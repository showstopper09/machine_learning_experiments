...
def extract_words(sent):
	...
	sent=re.sub(r'(\w)\'(\w)','\1\2',sent) #remove apostropes
	sent=re.sub(r'\W',' ',sent)#remove puntuations
...
#delete unwanted stuff to free up memory
model.delete_temporary_training_data(keep_inference=True)
model.save('reviews.d2v')

model.infer_vector(extract_words("this is not worth your time"))
from sklearn.metrics.pairwise import cosine_similarity
cosine_similarity([model.infer_vector(extract_words("absolutely ridiculous!"))],[model.infer_vector(extract_words("this is not worth your time"))])
cosine_similarity(
